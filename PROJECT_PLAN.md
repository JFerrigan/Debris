...
Current B.3R result: the [8,192-grain physics/performance viability checkpoint](docs/evidence/B3R-physics-viability.md) failed packed physical acceptance before timing could be measured. Review a revised convergence method against identical saved failures and the same fixture before additional candidate damage/fuel integration. The original [checkpoint contract](docs/PHYSICS_VIABILITY_PLAN.md) remains the reference.
