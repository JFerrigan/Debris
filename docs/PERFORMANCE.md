...
## B.3R 8,192-grain viability checkpoint — 2026-09-24

The [current checkpoint](evidence/B3R-physics-viability.md) ran an opt-in 8,192-active-grain layout on M4 Pro/Metal at 1280×800, v-sync off, no frame cap. It includes a 2,500-grain bay, 1,000-grain touching pile, 4,692 sparse exterior grains, 16 dynamic fragment slots, one anchored platform and 16 allocated terrain fields. All 4/2, 8/4 and 12/6 profiles faulted on solid penetration after 0, 0 and 1 committed combined ticks. **No profile reached the 120-frame warmup or supplied 600 valid measured frames.** Physics GPU, total GPU, frame and CPU submission p95 and steady-state allocations are unmeasured; there is no throughput pass or comparison to historical different workloads. Explicit solver plus terrain/render buffers were 83,751,624 bytes (79.87 MiB), within the 128 MiB resource budget alone. The procedural terrain rendering is a controlled equivalent, and GPU timing frame mapping remains unvalidated. Canonical packed cases also crossed the original .001 target and later faulted at the interim .002 grain/solid runtime limit; rigid/solid remains .001.

The 2026-09-24 1.8 position over-relaxation experiment used the same player matrix. Combined 4/2, 8/4 and 12/6 faulted at attempted ticks 1, 2 and 2, after 0, 1 and 1 committed ticks. The experimental and restored builds each used 83,751,624 explicit buffer bytes; the prototype added no dispatches or buffers. Both builds faulted before warmup, so the prototype's GPU cost and every p95 budget remain **unmeasured**. The retained benchmark now checks delayed physics samples against their submitted frame and one marker block; an unmatched total GPU frame result is explicitly unmeasured. See [decision and raw runs](evidence/B3R-physics-viability.md).

The later full-graph Jacobi relaxation prototype increased position sweeps from 2/4/6 to 16/32/48 while retaining the 4/8/12 velocity tiers, contact arithmetic, bodies, buffers and fault thresholds. It was rejected. In the same 8,192-grain Mac player fixture, 4/2, 8/4 and 12/6 faulted at attempted ticks 2, 3 and 4, after only 1, 2 and 3 committed ticks. Canonical packed cases also failed. The matching restored player again faulted at attempted ticks 1, 1 and 2, after 0, 0 and 1 commits. Both builds reported 83,751,624 explicit buffer bytes. The added 14/28/42 position sweeps per substep have an exact dispatch-count cost, but actual physics GPU, total GPU, frame and CPU submission p95 remain **unmeasured**: zero profiles reached warmup or the 600-frame timing window. Physics delayed-frame validation remains unexercised; total GPU source-frame attribution is still unverified. The [checkpoint evidence](evidence/B3R-physics-viability.md) contains saved-state and player results.
...
Planned explicit pool allowances total 111 MiB, leaving 17 MiB under the existing 128 MiB ceiling. This includes full reserved capacities, Krylov workspace, two contact caches, native timestamp buffers and bounded topology scratch; it excludes no allocated inactive pool. One solver is instantiated at a time. Actual allocation, command/driver memory and process footprint must be recorded separately. The 111 MiB plan is not a measured resource pass.

V2-1 requires a complete masked command schedule at <=1 ms GPU p95 and <=.25 ms CPU submission p95 for at least one profile, plus active operator-product costs on the actual frozen graph. These are feasibility limits within the unchanged final budgets. The exact live fixture remains 8,192 grains, 17 finite bodies, one anchor and 16 terrain chunks. Qualification requires the same profile to pass all correctness cases and 120 accepted warmup + 600 accepted measured frames: physics GPU <=8 ms, total GPU <=12 ms, frame <=20 ms, CPU submission <=2 ms, buffers <=128 MiB and zero steady-state managed submission allocations.

Three result types stay distinct: isolated operator/schedule cost, repeated failed-attempt replay cost, and live qualifying throughput. Faulting replays retain faults and reset explicitly; they can expose cost but cannot qualify a live workload. Missing timing is unmeasured. The planned tagged Metal timestamp bridge verifies source-frame identity for physics and the complete controlled frame; its inclusive `totalGpuFrameSpan` includes queue gaps and marker overhead. Until full frame coverage is established against a GPU capture, total GPU remains unmeasured. Existing unmatched FrameTimingManager observations cannot substitute for that evidence.
