...
## Recorded first-three review and adjustment

The [one-time review](evidence/B3R-first-three-review.md) is complete. Packed acceptance failed in that proof; later limited candidate integration proceeded under an interim tolerance. The current assignment returns to the [physics/performance viability checkpoint](PHYSICS_VIABILITY_PLAN.md) before more damage/fuel work. Lower broad-test counts do not establish improvement or usage savings. For a new Metal solver kernel batch, first run compiler/layout and isolated-contact smoke checks, inspect errors and repeated warnings together, then expand to packed fixtures. Check editor ownership/stale test processes before launch. All existing final correctness, conservation and build/player requirements remain. Do not repeat this review as a new planning layer.
