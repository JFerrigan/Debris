...
## Required contact-solver correction (B.3R)

The current ship/fragment candidate-pose rejection preserves geometry but incorrectly treats every loose cell as an immovable obstacle. Replace it with GPU-owned force integration and finite-mass contact impulses before expanding active budgets. Gather swept, deduplicated contacts; resolve stable contact islands/tiles without racing shared body velocity writes; then integrate corrected poses and publish compact state. Dynamic cell/fragment mass remains finite while sleeping. Static inverse mass is reserved for explicitly anchored bodies. CPU flight and save capture must stop interpreting any collision flag as a command to zero all velocity. Details and acceptance criteria are in [CONTACT_PHYSICS](CONTACT_PHYSICS.md); this is planned work, not a completed GPU capability.
