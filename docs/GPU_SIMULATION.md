...
## Required contact-solver replacement (B.3R)

The [locked parallel redesign](CONTACT_PHYSICS.md) supersedes the prototype layout and solver above: 48-byte world-space rotating square grains, parallel mass-split Jacobi impulses, separate position correction, two-cell bins, cached boundaries and a bounded rigid-only sequential solver. Candidate contacts/adjacency are gathered once per substep; reactions use reductions without float atomics. MatterSession owns committed/working state and ordered command submission. Whole-tick failure never consumes material/fuel. Prove the three locked profiles in the Mac player before gameplay/persistence cutover; then remove serial retries and dual cargo occupancy. B.GATE stays open.
