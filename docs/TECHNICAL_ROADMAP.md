...
| M4 — starter ship | blueprint authoring, free-drawn hull/structural prefabs/atomic units, arcade flight/fuel mass, drill, suction, cavity/door, physical cargo/spill | controlled cargo showcase | `SHIP_SYSTEM`, `COMPONENT_SYSTEM`, `UNITY_INTEGRATION`, `GAME_DESIGN` |
...
| M7 — strategic/home loop | Frontier Count, contacts, persistent/temporal encounter split, entry/exit transition, home landing/hub, storage/sale/debt, fuel risk UI | full first-playable loop | `ARCHITECTURE`, `GAME_DESIGN`, `STATUS` |
...
| M9 — field operations | Arcturus body, basic booster, exit/return ship, welding, misc storage, hub walking | leave ship, patch hull, return without inventory duplication | `FIELD_OPERATIONS_AND_COMBAT`, `INPUT_AND_CAMERA`, `PERSISTENCE` |
...
1. Open installed Unity 6000.3.11f1 (3000ef702840) and verify package resolution.
