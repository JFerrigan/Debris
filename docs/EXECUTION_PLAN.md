...
Current priority (measured 2026-09-24 UTC): the bounded [physics/performance viability checkpoint](evidence/B3R-physics-viability.md) found a physical correctness stop before throughput sampling. All three profiles reject packed cargo and the 8,192-active-grain workload under current runtime limits. The trace/reference replay supports insufficient packed-position convergence at locked iteration counts, without isolating a narrow arithmetic defect. Next: review one revised convergence method and its contract/cost, then retest the same fixture. Keep R1 and B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred. The assignment stops at this evidence handoff.
