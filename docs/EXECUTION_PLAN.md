...
Current priority (measured 2026-09-24 UTC): the bounded [physics/performance viability checkpoint](evidence/B3R-physics-viability.md) still stops on packed correctness before throughput sampling. A 1.8 scalar position over-relaxation prototype failed all three profiles and was rejected; original solver/math and runtime limits are restored. Next: test a stronger coupled-position method on identical saved states, define any changed numerical/iteration contract and measured GPU cost, then retest the same 8,192-grain fixture. Keep R1 and B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred. This assignment stops at the evidence decision.
...
- [ ] B.3R.R1 Parallel square-grain proof. The [viability checkpoint](evidence/B3R-physics-viability.md) and one rejected 1.8 over-relaxation prototype failed: all three profiles fault before the 8,192-grain warmup completes, and original .001 packed acceptance also fails. Throughput remains unmeasured. Grain/solid runtime rejection is .002; rigid/solid remains .001. A stronger coupled-convergence prototype and same-fixture retest are next. Full R1 correctness/performance coverage is incomplete.
