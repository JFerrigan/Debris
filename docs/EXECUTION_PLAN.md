...
Current priority (measured 2026-09-24 UTC): the bounded [physics/performance viability checkpoint](evidence/B3R-physics-viability.md) still stops on packed correctness before throughput sampling. Scalar 1.8 over-relaxation and fourfold/eightfold full-graph Jacobi position sweeps failed all three profiles and were rejected; original solver/math and runtime limits are restored. Next: isolate the archived wall residual and prototype one different coupled position solve with a defined mass/iteration contract and measurable GPU cost, then retest the same 8,192-grain fixture. Keep R1 and B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred. This assignment stops at the evidence decision.
...
- [ ] B.3R.R1 Parallel square-grain proof. The [viability checkpoint](evidence/B3R-physics-viability.md), rejected 1.8 over-relaxation and rejected deeper full-graph position sweeps failed: all three profiles fault before the 8,192-grain warmup completes, and original .001 packed acceptance also fails. Throughput remains unmeasured. Grain/solid runtime rejection is .002; rigid/solid remains .001. A different coupled-position method and same-fixture retest are next. Full R1 correctness/performance coverage is incomplete.
- [ ] B.3R.R2 Candidate mining/cargo/door/fuel/damage and exceptional limits. Limited opt-in cutter, effective door collision, whole-square cavity classification, compact capacity admission and mounted suction are implemented with scoped test acceptance. Player suction input remains unverified. Physical packed-capacity acceptance, damage/fuel and remaining exceptional cases are incomplete. Further damage/fuel work remains paused pending a revised convergence decision.
