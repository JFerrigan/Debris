...
Current priority (2026-09-25 UTC): implement the [coupled contact solver V2 architecture](CONTACT_SOLVER_V2.md) through its [explicit batch gates](CONTACT_SOLVER_V2_IMPLEMENTATION.md). [V2-0A](evidence/B3R-v2-convergence.md) passed exact pre-step archive/replay for nine unchanged legacy failures. The next bounded task is V2-0B independent Float64 geometry and tiny direct contact reference; V2-0C converged packed reference completes the full V2-0 decision. The legacy solver still fails [packed acceptance](evidence/B3R-physics-viability.md). Keep R1/B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred until their stated prerequisites pass.
