...
Authorized scope: implement phases A–E in order through playable alien-drone escalation, subject to the latest bounded user assignment below. Continue automatically when each gate passes unless that assignment specifies a handoff. Final story resolution is outside this run. Follow [AGENTS.md](../AGENTS.md) for validation and concise context loading. Preserve existing work, make coherent subsystem commits, and push ordinary commits to origin/main.

Current priority (design revision 2026-09-24 UTC): implement the [coupled contact solver V2 architecture](CONTACT_SOLVER_V2.md) through its [explicit batch gates](CONTACT_SOLVER_V2_IMPLEMENTATION.md). The legacy solver and its relaxation/sweep experiments still fail the [packed checkpoint](evidence/B3R-physics-viability.md); the last experiment changed iteration counts, not coupling. V2 is planned, not implemented or qualified. The current user assignment is documentation only and stops after the reviewed plan. The next Claude implementation task is [V2-0A](CLAUDE_V2_0A_PLAN.md): exact pre-step capture and reproducible failed-attempt replay, then stop. V2-0B independent geometry/tiny direct reference and V2-0C converged packed reference complete V2-0 in subsequent assignments. Keep R1/B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred until their stated prerequisites pass.

Historical user scope (2026-09-17): finish the interrupted diagnostic work and deliver a working Mac build only. That checkpoint is complete. The user accepted the measured penetration for that checkpoint; the numerical physics gate and gameplay cutover remained open. See [diagnostic/build evidence](evidence/B3R-packed-diagnosis.md).

Historical startup investigation (2026-09-18 UTC): the single permitted candidate player launch reproduced `Page`. Grain-only page validation and empty-population readback were corrected; 11 focused tests and 110 fast-suite tests passed (one explicit skip), and the Mac rebuild succeeded. No second player launch in that investigation; its post-fix player acceptance was unverified under that task's limit. See [startup evidence](evidence/B3R-page-startup.md). Later scoped candidate acceptance is recorded below.

Current B.3R.R2 checkpoint (2026-09-21 UTC): limited [candidate drilling](CANDIDATE_DRILLING_PLAN.md), effective door collision, GPU cavity classification/capacity admission and mounted suction are accepted under `-debrisParallelGameplay`: terrain ownership, local boundary-run cache, compact oriented-square placement checks, one ordered terrain/cache/grain publication and topology fencing passed. Door opening replaces the ship cache after the fence; closing takes one command-time snapshot and stays open for an oriented-square obstruction. Cavity classification updates flags only for whole oriented squares and publishes per-material count and rejected-admission facts without a normal full snapshot. Suction derives force from current GPU body pose and acts only through the exterior mouth strip. Legacy remains the default. Damage/fuel transfers, persistence and travel remain separate R2-R3 work.
...
- [ ] B.3R.R1 Coupled square-grain proof. Legacy 4/2, 8/4 and 12/6 remain failed comparators; throughput is unmeasured. The [V2 design](CONTACT_SOLVER_V2.md) replaces degree-split Jacobi and separate rigid sweeps with a global contact system, true two-point manifolds and fresh coupled geometric correction. V2 requires original .001 solid acceptance; old runtime .002 is historical only.
- [ ] B.3R.R1.V2-0 Exact replay inputs, independent geometry, direct analytical contact checks and converged packed reference; explicit formulation decision before GPU solver implementation.
  - [ ] V2-0A Immutable exact pre-step archives, nine-case legacy capture/replay and unchanged physical rollback; bounded next Claude session stops here.
  - [ ] V2-0B Independent Float64 geometry and tiny direct analytical contact reference.
  - [ ] V2-0C Independent converged packed reference and separate V2 double residual/solve comparison; only then decide full V2-0.
...
- [ ] B.3R.R2.V2-4 Qualified solver supports opt-in candidate flight/drilling/doors/cargo/suction, fenced topology and actual player-input acceptance.
- [ ] B.3R.R2 Candidate mining/cargo/door/fuel/damage and exceptional limits. Existing limited candidate behavior must be ported and reverified at V2-4. Physical packed capacity, damage/fuel, input and remaining exceptional cases stay open. Damage/fuel may resume only after V2-3 and V2-4 pass; planned transaction details are not acceptance.
- [ ] B.3R.R3 Schema-5/schema-3 migration, travel and cold restoration; establish full lifecycle evidence before default cutover.
- [ ] B.3R.R4 Verify final B.GATE, explicitly switch the default and delete superseded runtime architecture. See [CONTACT_PHYSICS](CONTACT_PHYSICS.md).
