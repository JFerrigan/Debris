...
Current priority (plan prepared 2026-09-23 UTC): perform the bounded [physics/performance viability checkpoint](PHYSICS_VIABILITY_PLAN.md) before further damage/fuel integration. [Terra handoff prompt](TERRA_PHYSICS_CHECKPOINT_PROMPT.md). Measure the 8,192-active-grain workload, investigate .001 packed convergence and long-run interim bounds, correct a demonstrated defect within the locked architecture if feasible, then publish a decision and stop at the checkpoint handoff. This documentation batch has not executed that work. Keep R1 and B.GATE open, legacy default, and damage/fuel plus R3–R4 deferred.
...
- [ ] B.3R.R1 Parallel square-grain proof. **Current work:** [physics/performance viability checkpoint](PHYSICS_VIABILITY_PLAN.md). The original .001-cell target and 8,192-grain throughput remain unresolved. Interim integration used .002 for grain/solid rejection; current rigid/solid code remains at .001. Full R1 correctness/performance coverage is incomplete. [Historical evidence](evidence/B3R-parallel-pieces.md).
- [ ] B.3R.R2 Candidate mining/cargo/door/fuel/damage and exceptional limits. Limited opt-in cutter, effective door collision, whole-square cavity classification, compact capacity admission and mounted suction are implemented with scoped test acceptance. Player suction input remains unverified. Physical packed-capacity acceptance, damage/fuel and remaining exceptional cases are incomplete. Further damage/fuel work is deferred until the viability checkpoint decision.
