...
Current handoff: **the solver V2 architecture is planned; implementation has not started.** The [design](CONTACT_SOLVER_V2.md) specifies a global matrix-free Newton/GMRES contact solve, two-point manifolds and fresh coupled position correction, with fixed numerical, memory and timing contracts. The [implementation plan](CONTACT_SOLVER_V2_IMPLEMENTATION.md) defines each gate and stop condition. **Next Claude task: V2-0A exact committed-state archives and failing-attempt replay**, following the [bounded implementation plan](CLAUDE_V2_0A_PLAN.md) and [copy-ready prompt](CLAUDE_V2_0A_PROMPT.md). Stop after replay evidence; V2-0B geometry/tiny reference and V2-0C packed convergence follow in separate assignments. Full V2-0 must pass before GPU implementation. The [design evidence](evidence/B3R-v2-design.md) records this documentation-only checkpoint. R1/B.GATE remain open; damage/fuel stays paused.
