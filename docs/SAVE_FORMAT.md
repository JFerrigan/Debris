...
World collection now preserves the union of both verified manifest/index dependency graphs. Obsolete index/site/blob files and abandoned pending files can be reclaimed after publication; any damaged live dependency defers collection before the first deletion. The two recoverable world generations, all their site records and unrelated files remain intact. Retention no longer grows with every successful save solely because old roots became unreachable.
