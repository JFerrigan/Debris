...
namespace Debris.Simulation
{
    ...
    internal sealed class CandidateTerrainTransaction : IDisposable
    {
        ...
        {
            if(disposed||faulted||Busy||value==null||boundaries==null||!terrain.IsCurrent(value)||!float.IsFinite(density)||density<=0)return CandidateEditStatus.Unavailable;
        ...
        {
            if(disposed||edit==null||awaiting||placementStatus!=0)return CandidateEditStatus.StaleEdit;
            if(faulted)return CandidateEditStatus.Faulted;
            if(!terrain.IsCurrent(edit))return CandidateEditStatus.StaleEdit;
