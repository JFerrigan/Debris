...
namespace Debris.Presentation
{
    public sealed class Showcase : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if(ship!=null)GUI.Label(new Rect(30,145,650,70),parallelGameplay?$"CANDIDATE ACTIVE   /   {active:N0} PHYSICAL GRAINS   /   FLIGHT ENABLED":$"FUEL {ship.Fuel.Energy:F1}   /   CARGO {session.ShipStats[2].x:N0} / 2,500   /   DOOR {(session.ShipStats[2].z>0?"OPEN":"CLOSED")}",label);
